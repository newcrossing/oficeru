<?php
/**
 * PEAR_Frontend, the singleton-based frontend_old for user input/output
 *
...
/**
 * Singleton-based frontend_old for PEAR user input/output
 *
 * @category   pear
 * @package    PEAR
 * @author     Greg Beaver <cellog@php.net>
...
{
    /**
     * Retrieve the frontend_old object
    ...
    /**
     * Set the frontend_old class that will be used by calls to {@link singleton()}
     *
    ...
    public static function &setFrontendClass($uiclass)
    {
        ...
        if (class_exists($uiclass)) {
            ...
            // important frontend_old methods
            ...
            $err = PEAR::raiseError("not a frontend_old class: $uiclass");
            return $err;
        }
    ...
    public static function &setFrontendObject($uiobject)
    {
        ...
        if (!is_a($uiobject, 'PEAR_Frontend')) {
            $err = PEAR::raiseError('not a valid frontend_old class: (' .
                get_class($uiobject) . ')');
            return $err;
        }
     ...
     *
     * A frontend_old that requires multiple requests to retrieve and process
